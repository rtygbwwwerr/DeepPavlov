...
#         print("Batch:{}, input shape:{}, output shape:{}".format(counter, x_batch.shape, y_batch.shape))
        counter += 1
...
@register('keras_classifier_model')
class KerasClassifier(Serializable):
    """
    ...
    def gen_callbacks(self):
        ...
        file_name = "/wights-{}-s{}".format('CNNNet1D', 1)
        checkpoint = callbacks.ModelCheckpoint('../' + "cls" + '-{epoch:02d}-{acc:.4f}-{val_acc:.4f}.h5', monitor='val_acc',
                                               save_best_only=False, save_weights_only=True, verbose=1)
        lr_decay = callbacks.LearningRateScheduler(schedule=lambda epoch: self.opt['learning_rate'] * (self.opt['learning_rate_decay'] ** epoch))
    ...
    def fit_batches(self, data_train, data_valid, batch_size, epochs):
        """
        Train the model on the given batch

        Args:
            x: list of tokenized text samples
            y: list of labels
        ...
        x_train = self.pad_texts(data_train[0])
        y_train = np.squeeze(np.array(data_train[1]))
...
#         print(y_train)
        ...
        x_valid = self.pad_texts(data_valid[0])
        y_valid = np.squeeze(np.array(data_valid[1]))
        print("input shape:{}, output shape:{}, batch size:{}".format(x_train.shape, y_train.shape, batch_size))
        # Training with data augmentation. If shift_fraction=0., also no augmentation.
        self.model.fit_generator(generator=data_generator(x_train, y_train, batch_size),
                            steps_per_epoch=int(y_train.shape[0] / batch_size),
                            epochs=epochs,
                            validation_data=[x_valid, y_valid],
                            callbacks=self.gen_callbacks())
...
#                       loss=loss)
        ...
        model.compile(optimizer=self.optimizer,
                    loss=loss,
                    metrics=["accuracy"]
    #                       metrics={'capsnet': 'accuracy'}, binary_accuracy
                      )
        return model
...
#                                                                        K.eval(self.optimizer.decay) * self.batches_seen)
    ...
    def bi_fast_gru_model(self, units_lstm: int = 256, 
                          layer_num: int = 2, dense_size: int = 128,
                    coef_reg_lstm: float = 0., coef_reg_den: float = 0.,
                    dropout_rate: float = 0., rec_dropout_rate: float = 0.,
                    input_projection_size: Optional[int] = None, **kwargs) -> Model:
        """
        ...
        for i in range(layer_num):
            output = Bidirectional(CuDNNGRU(units_lstm,
                                       return_sequences=True,
                                       kernel_regularizer=l2(coef_reg_lstm)
                                       ))(output)
